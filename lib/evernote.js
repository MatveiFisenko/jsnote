...
//			console.log(data);
	...
	getNoteContent: function(e, data) {
		...
		if (e) {
			...
			E.data.notes[E.data.notes.pending].title = $('#' + noteId[0]).html();//save note title
		...
		if (data.guid === E.data.notes.pending) {//check if no other note was asked before this content arrived
			var content = E._setPendingNoteContent(data.content);
		}
		else {
	...
	updateNote: function(e, data) {
		if (e) {
			...
			if (E.data.notes.active.substr(0, 8) === 'createNo'
				&& (E.data.notes[E.data.notes.active].sha1Pending !== E.data.notes[E.data.notes.active].sha1)
			...
			E.data.notes[E.data.notes.active].sha1Pending = hex_sha1(content);//we need to know last pending sha1 to ensure save update

			if (E.data.notes[E.data.notes.active].sha1Pending === E.data.notes[E.data.notes.active].sha1) {//no update needed
				return;
			}
			...
			return E.getPhpData(E.updateNote, 'updateNote',
				{ guid: E.data.notes.active,
					content: content.replace(titleRegex, '').replace(tagsRegex, ''),
					...
					sha1: E.data.notes[E.data.notes.active].sha1Pending
				});
		}
		...
		if (E.data.notes[data.guid].sha1Pending === data.sha1) {
			E.data.notes[data.guid].sha1 = E.data.notes[data.guid].sha1Pending;
	...
	createNote: function(e, data) {
		if (e) {
			...
			E.data.notes[E.data.notes.pending] = { title: 'Title', counter: 0 };

			E.getNoteContent(null, { guid: E.data.notes.pending, content: '<p> </p>', tags: [] });
		}
	},
	...
	_setPendingNoteContent: function(content) {
		...
		//TODO possible we do not need this content 'set/get' before sha1 check
		content = $('#noteContents').html();

		E.data.notes[E.data.notes.pending].sha1 = hex_sha1(content);//calculate content hash for future updates
