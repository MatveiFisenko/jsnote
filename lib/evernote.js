...
//			console.log(data);
	...
	getNoteContent: function(e, data) {
		...
		var tags = '', title = data.title || $('#' + E.data.activeNote.guid).html();
		$.each(data.tags, function(i, element) { tags += element + ', '; });

		$('#noteContents').html(data.content + '<p>noteTitle: ' + title + '</p><p>noteTags: ' + tags.slice(0, -2) + '</p>');
	...
	updateNote: function(e, data) {
		...
		//TODO if we save new note and then fast create new one - previous new note will not be in history tab
	...
	_historyTabs: function(guid, title) {
		//get history ul
		...
		if (!oHistory.length) {
			oHistory = $('<ul class="tabs noteHistoryTabs"></ul>').insertBefore('#noteContents')
				.click(function(e) {
					if ($(e.target).is('a[class!=current]')) {
						e.target.id.match(/^hcreateNote$/) ? E.createNote(e) : E.getNoteContent(e);
					}
				});
		}
	...
	_getAuthData: function(force) {
		...
		else if (document.cookie.match(/oauth_token=([^;]+)/) && document.cookie.match(/edam_shard=([^;]+)/)) {
