...
var E = {
	...
	data: {
		...
		_system: {
			dataAdapterPhp: '/php/evernote.php',

			noteTitlePrepend: 'noteTitle:',
			noteTagsPrepend: 'noteTags:'
		}
	},
	...
	oAuth: {
		data: {
			...
			oauth_callback: 'http://' + window.location.hostname + '/userAuthSuccess.html'
		},
	...
	getPhpData: function(callback, action, params) {
		...
		$.post(E.data._system.dataAdapterPhp, $.extend({
...
//			console.log(data);
	...
	findNotes: function(e, data) {
		...
		$.each(data.notes, function(i, element) {
			...
			//save note data for future use - open content/update
			if (!E.data.notes[element.guid]) {
				E.data.notes[element.guid] = { title: element.title, tagGuids: element.tagGuids || [], updateSequenceNum: element.updateSequenceNum };
			}
			//if we have this note already but note last version
			else if (E.data.notes[element.guid].updateSequenceNum !== element.updateSequenceNum) {
				E.data.notes[element.guid].title = element.title;
				E.data.notes[element.guid].tagGuids = element.tagGuids;
				E.data.notes[element.guid].updateSequenceNum = element.updateSequenceNum;
			}
		});
		...
		//be sure to display search results
		$('#filterHolder').animate({ scrollTop: 0 });
	},
	...
	getNoteContent: function(e, data) {
		...
		if (e) {
			...
			//if have this note is in cache & cache is up-to-date
			if ($.jStorage.get('E.note.usn' + E.data.notes.pending) === E.data.notes[E.data.notes.pending].updateSequenceNum) {
		...
		data.content = data.content.replace(/<en-media.*?hash="(\w+)".*?\/>/gm, function (str, p1) {
			...
			//add title & tags
			+ '<p>' +  E.data._system.noteTitlePrepend + ' ' + title + '</p><p>'
			+  E.data._system.noteTagsPrepend + ' '  + tags.slice(0, -2) + '</p>';

		if (data.guid === E.data.notes.pending) {//check if no other note was asked before this content arrived
			E._setPendingNoteContent(data.content);
		}
		else {
		...
		$.jStorage.set('E.note.' + data.guid, data.content);
		//save it's updateSequenceNum in browser storage
		$.jStorage.set('E.note.usn' + data.guid, E.data.notes[data.guid].updateSequenceNum);
	},
	...
	updateNote: function(e, data) {
		if (e) {
			...
			//create <a href> tags from urls in text, it will be in note editor after note reload (reopen)
			content = content.replace(/(<p>|\s)(https?:\/\/\S+?)(<\/p>|\s)/gm, '$1<a href="$2">$2</a>$3');
			...
			var titleRegex = new RegExp('<p>' +  E.data._system.noteTitlePrepend + '(.*?)<\/p>', 'm'),
				trim = /(^\s*(&nbsp;)*\s*)|(\s*(&nbsp;)*\s*$)/g,//remove whitespace and &nbsp;
				tagsRegex = new RegExp('<p>' +  E.data._system.noteTagsPrepend + '(.*?)<\/p>', 'm'),
		...
		//update note title & tags
		E.data.notes[data.guid].title = data.title;
		E.data.notes[data.guid].tagGuids = data.tagGuids;
		E.data.notes[data.guid].updateSequenceNum = data.updateSequenceNum;
		//update updateSequenceNum in browser storage
		$.jStorage.set('E.note.usn' + data.guid, data.updateSequenceNum);

		//update note title on history tab
	...
	_getResourceByHashURL: function(guid, hash) {
		...
		return E.data._system.dataAdapterPhp + '?' + $.param({
			oauth_token : authData.token,
			edam_shard : authData.shard,
			module : 'note',
	...
	_setPendingNoteContent: function(content) {
		$('#noteContents').html(content);
		//TODO possible we do not need this content 'set/get' before md5 check
		//calculate content hash for future updates
		E.data.notes[E.data.notes.pending].md5Pending = E.data.notes[E.data.notes.pending].md5 = hex_md5($('#noteContents').html());

		E._historyTabs(E.data.notes.pending, E.data.notes[E.data.notes.pending].title);
...
var Auth = {
	...
	makeUserAuth: function() {
		$('body').prepend('<iframe src="' + E.oAuth.links.userAuthorizationURL + '?'
			+ $.param({ format: 'microclip', oauth_token: E.oAuth.data.oauth_token })
			+ '" class="auth"></iframe>');

		$.notify.show('Auth permission dialog: completed');
	},

	userAuthSuccess: function(successParams) {
		$.notify.show('Auth ask permission: completed');

		$('body > iframe.auth').remove();

		//get oauth_verifier from query string
		E.oAuth.data.oauth_verifier = successParams.match(/oauth_verifier=([^&]+)/)[1];
		...
		$.get(E.oAuth.links.accessTokenURL, E.oAuth.data, function(data) {
			...
			//delete oauth_verifier too as we'll get new one after successful user auth
			delete E.oAuth.data.oauth_verifier;
			//decode received data because evernote wants clean oauth_token, not like: S%3Ds4...
