...
//			console.log(data);
	...
	getNoteContent: function(e, data) {
		...
		if (e) {
			...
			if ($('#h' + E.data.activeNote.guid).length) {
				//contents can be not set! It happens when we add new note than very fast add another one
				//first note contents now are under E.note.createNote key and can not be trusted
				var contents = $.jStorage.get('E.note.' + E.data.activeNote.guid);

				if (contents) {
					$('#noteContents').html(contents);
	...
	updateNote: function(e, data) {
		if (e) {
			...
			contents = contents.replace(/<img.*?id="(\w+)".*?\/>/gm, function (str, p1) {
